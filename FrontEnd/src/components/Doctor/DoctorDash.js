...
import DoctorMedical from "../MedicalRecord/DoctorMedical";

/**
 * DoctorDash Component
 * 
 * This is the main dashboard component for doctors in the healthcare system.
 * It provides a comprehensive interface for doctors to manage their practice,
 * view appointments, access medical records, and update their profile information.
 * 
 * Features:
 * - Dashboard overview with statistics
 * - Appointment management (integrated with DoctorAppointments component)
 * - Medical records management (integrated with DoctorMedical component)
 * - Profile management with edit capabilities
 * - Responsive Bootstrap-based UI
 * - Authentication and authorization checks
 * 
 * @component
 * @returns {JSX.Element} The rendered doctor dashboard
 */
function DoctorDash() {
  ...
  /**
   * State for storing doctor's profile information fetched from API
   * Contains: doctorId, userID, docName, phoneNo, email, speciality
   */
  ...
  /**
   * Array of today's appointments for the doctor
   * Used for dashboard statistics and today's schedule display
   */
  const [todaysAppointments, setTodaysAppointments] = useState([]);
  
  /**
   * Array of future appointments for the doctor
   ...
   * Fetches initial data: doctor profile and appointments
   */
  useEffect(() => {
    fetchDoctorProfile();
  }, []);
  ...
  /**
   * Fetches the doctor's profile information from the backend API
   * Handles authentication, makes API call, and updates state accordingly
   * 
   * API Endpoint: GET /api/Doctor/GetDoctorProfile
  ...
  const fetchDoctorProfile = () => {
    ...
    axios
      .get("http://localhost:5043/api/Doctor/GetDoctorProfile", {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then((response) => {
        console.log("Doctor profile fetched successfully:", response.data);
      ...
      .catch((error) => {
        ...
        if (error.response?.status === 403) {
          ...
          setError("Failed to load doctor profile. Error: " + (error.message || "Unknown"));
        }
        setLoading(false);
      });
  };
   ...
   * 
   * API Endpoint: GET /api/Doctor/GetAppointmentsByDoctor
   * Authentication: JWT token from sessionStorage
   */
  const fetchStatistics = () => {
    const token = sessionStorage.getItem("token");
    if (!token || !doctorProfile?.doctorId) {
      console.log("Token or doctorId missing, skipping fetchStatistics");
      return;
    }
    
    console.log(`Fetching appointments for doctorId: ${doctorProfile.doctorId} at ${new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' })}`);
    axios
      ...
      .then((response) => {
        ...
        const today = new Date().toISOString().split('T')[0]; // "2025-08-18" for today
      ...
      .catch((error) => {
        ...
        if (error.response?.status === 404) {
          console.log("Endpoint not found or no appointments for this doctor");
   ...
   * 
   * API Endpoint: PUT /api/Doctor/UpdateDoctorByID/{doctorId}
   * Authentication: JWT token from sessionStorage
   * 
  ...
  const handleSaveProfile = () => {
    ...
    if (!profileForm.docName || !profileForm.phoneNo || !profileForm.email) {
      alert("Please fill in all required fields.");
      return;
    }

    console.log("Attempting to update doctor profile...");
    console.log("Doctor ID:", doctorProfile.doctorId);
    console.log("Form data:", profileForm);

    const updatePayload = {
      doctorId: doctorProfile.doctorId,
      userID: doctorProfile.userID,
      docName: profileForm.docName,
    ...
    console.log("API endpoint:", `http://localhost:5043/api/Doctor/UpdateDoctorByID/${doctorProfile.doctorId}`);

    axios
      .put(
        `http://localhost:5043/api/Doctor/UpdateDoctorByID/${doctorProfile.doctorId}`,
        updatePayload,
        { headers: { Authorization: `Bearer ${token}` } }
      )
      .then((response) => {
    ...
    <div className="min-vh-100 bg-light">
      ...
      <div className="bg-primary text-white p-4 shadow">
        <div className="d-flex justify-content-between align-items-center">
          <div>
            <h1 className="h3 mb-1">👨‍⚕️ Doctor Dashboard</h1>
            {doctorProfile && (
              <p className="mb-0">Welcome back, <strong>{doctorProfile.docName}</strong>!</p>
            )}
          </div>
      ...
      <div className="bg-white p-3 border-bottom shadow-sm">
        <div className="d-flex flex-wrap gap-2">
          {[
            ...
            { id: "appointments", label: "📅 Appointments", color: "primary" },
      ...
      <div className="p-4">
        ...
        {activeTab === "appointments" && (
          <DoctorAppointments doctorId={doctorProfile?.doctorId} />
        )}

        {/* ===== MEDICAL RECORDS TAB ===== */}
        {activeTab === "medical-records" && (
          <DoctorMedical doctorId={doctorProfile?.doctorId} />
        )}
      </div>
                ...
                {doctorProfile && (
                  <div>
                    {!isEditingProfile ? (
                      ...
                      <div>
                        <div className="mb-3">
                          <strong>Doctor ID:</strong> {doctorProfile.doctorId}
                        </div>
                        <div className="mb-3">
                          <strong>User ID:</strong> {doctorProfile.userID}
                        </div>
                        <div className="mb-3">
                          <strong>Name:</strong> {doctorProfile.docName}
                        </div>
                        <div className="mb-3">
                          <strong>Speciality:</strong> {doctorProfile.speciality}
                        </div>
                        <div className="mb-3">
                          <strong>Phone:</strong> {doctorProfile.phoneNo}
                        </div>
                        <div className="mb-4">
                      ...
                      <div>
                        <div className="mb-3">
                          ...
                          <input
                            ...
                            value={profileForm.docName}
                            onChange={(e) => setProfileForm({ ...profileForm, docName: e.target.value })}
                        ...
                        <div className="mt-3 p-2 bg-light rounded">
                          <small className="text-muted">
                            ...
                            Form Name: {profileForm.docName}<br />
                            Form Phone: {profileForm.phoneNo}<br />
                            Form Email: {profileForm.email}<br />
                            Current Profile Name: {doctorProfile.docName}<br />
                            Current Profile Phone: {doctorProfile.phoneNo}<br />
                            Current Profile Email: {doctorProfile.email}<br />
                            Doctor ID: {doctorProfile.doctorId}<br />
                            User ID: {doctorProfile.userID}
                          </small>
                        </div>
                      </div>
                    )}
